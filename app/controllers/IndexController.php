...
class IndexController extends BaseController{
	...
	private function login_user($username , $password_text, $rememberme = false){
		...
		$db->where("StaffName", $username)->orWhere("Emailid", $username);
	...
	function register($formdata = null){
		if($formdata){
			...
			$db = $this->GetModel();
			$tablename = $this->tablename;
			$fields = $this->fields = array("StaffName","Password","Emailid","Designation"); //registration fields
			...
			if($this->validated()){
				...
				if($rec_id){
					$this->login_user($modeldata['Emailid'] , $password_text);
					return;
				}
				else{
					$this->set_page_error();
